...
for f in CL*.tsv; do 
	cut -d $'\t' -f 1 $f >> ids-"$f"
	cut -d $'\t' -f 2 $f >> ids-"$f"
	sort -u ids-"$f" >> ids-unq-"$f"  #keeping the only unique genomes from every CL*.tsv file
done 

for f in ids-unq-CL*.tsv; do 
    sed "s/^/$f\t/" "$f" >> ids-unq-clusters-comb_a.tsv  #replacing ids-unq- with nothing in ids-unq-clusters-comb_a.tsv;
done
 
sed "s/ids-unq-//" ids-unq-clusters-comb_a.tsv >> ids-unq-clusters-comb_b.tsv #appending contents in ids-unq-clusters-comb_b.tsv;
sed "s/.tsv//" ids-unq-clusters-comb_b.tsv >> ids-unq-clusters-comb.tsv  #appending contents in ids-unq-clusters-comb.tsv 
...
awk 'BEGIN {OFS="\t"}; {print $2,$3}'  | sort -k1 >> clusters_formatted_in_rows.tsv
...
if [ -n "$source_option" ]; then
	...
	grep "clinical" "$genomes_and_sources" | awk '{print $1}' > clinical_genomes.tsv  #extracting genomes from clinical sources

	sort -k 1,1 clinical_genomes.tsv > sorted_clinical_genomes.tsv
	sort -k 2,2 "$genomes_in_clusters" > sorted_genomes_in_clusters.tsv  #Sorting input files

	join -1 1 -2 2 -o 1.1,2.1 sorted_clinical_genomes.tsv sorted_genomes_in_clusters.tsv > clinical_genomes_in_clusters.tsv  #Combining clinical genomes and cluster information
	...
	rm clinical_genomes.tsv sorted_clinical_genomes.tsv sorted_genomes_in_clusters.tsv clinical_genomes_in_clusters.tsv  #deleting intermediate files
